			...
			<div class="col-12">
				<div class="rounded-3" style="@if ($lesson->lesson_type != 'text' && !in_array(getExtensionPath($lesson->doc_file), ['docx', 'doc'])) aspect-ratio:2/1;overflow:hidden @endif">
                    ...
                    @elseif ($lesson->lesson_type == 'document')
                      ...
                      <iframe src="{{ route('doc-viewer', $lesson->doc_file) }}" style="width: 100%;height: 100%;"></iframe>
                      @else
				...
				<div class="row g-4">
					...
					<div class="col-12">
						...
						<ul class="list-inline mb-0">
							<li class="list-inline-item h6 me-3 mb-1 mb-sm-0"><i class="fas fa-user-graduate text-orange me-2"></i>{{ $course->users->count() }} người tham gia</li>
						</ul>
					</div>
					<!-- Course title END -->
					...
					<div class="col-12">
						...
						<ul class="nav nav-pills nav-pills-bg-soft px-3" id="course-pills-tab" role="tablist">
							...
							<li class="nav-item me-2 me-sm-4" role="presentation">
								<button class="nav-link mb-0 active" id="course-pills-tab-1" data-bs-toggle="pill" data-bs-target="#course-pills-1" type="button" role="tab" aria-controls="course-pills-1" aria-selected="true">Nội dung</button>
							</li>
							<!-- Tab item -->
							<li class="nav-item me-2 me-sm-4" role="presentation">
								<button class="nav-link mb-0" id="course-pills-tab-2" data-bs-toggle="pill" data-bs-target="#course-pills-2" type="button" role="tab" aria-controls="course-pills-2" aria-selected="false">Tệp đính kèm</button>
							</li>
						</ul>
						<!-- Tabs END -->
							...
							<div class="tab-pane fade" id="course-pills-2" role="tabpanel" aria-labelledby="course-pills-tab-2">
								<ul class="list-group">
									...
									<li class="list-group-item d-flex justify-content-between align-items-center">
										...
										<a href="{{ route('doc-viewer', $val) }}" target="_blank" class="btn btn-link btn-success m-0"><i class="fas fa-download"></i></a>
									</li>
									@endforeach
								  </ul>
							</div>
							...
							<div class="accordion accordion-icon accordion-bg-light" id="accordionExample2">
								...
								<div class="accordion-item mb-3">
									<h6 class="accordion-header font-base" id="heading-{{$key}}">
										<a class="accordion-button fw-bold collapsed rounded d-block" href="#collapse-{{$key}}" data-bs-toggle="collapse" data-bs-target="#collapse-{{$key}}" aria-expanded="{{ $section->lessons->contains($lesson->id) ? 'true' : 'false' }}" aria-controls="collapse-{{$key}}">
											<span class="mb-0">{{ $section->title }}</span> 
											<span class="small d-block mt-1">({{ $section->lessons->count() }} bài học)</span> 
										</a>
									</h6>
									<div id="collapse-{{$key}}" class="accordion-collapse collapse {{ $section->lessons->contains($lesson->id) ? 'show' : '' }}" aria-labelledby="heading-{{$key}}" data-bs-parent="#accordionExample2">
										<!-- Accordion body START -->
										<div class="accordion-body mt-3">
											<div class="vstack gap-3">
                                                @foreach($section->lessons as $key=>$l)
												<!-- Course lecture -->
												<div class="d-flex justify-content-between align-items-center {{ $lesson->id == $l->id ? 'bg-light' : '' }}">
													<div class="position-relative d-flex align-items-center">
														<a href="{{ route('course-learn', ['id' => $course->id, 'slug' => $course->slug, 'lesson' => $l->id]) }}" class="btn btn-danger-soft btn-round btn-sm mb-0 stretched-link position-static">
															{!! getIconLesson($l) !!}
														</a>
														<span class="d-inline-block text-truncate ms-2 mb-0 h6 fw-light w-200px">{{$l->title}}</span>
													</div>
													...
													<div class="form-check">
														<input class="form-check-input mark-lesson" data-course-id="{{$course->id}}" type="checkbox" value="{{$l->id}}">
													</div>
													@endif
												</div>
                                                @endforeach
