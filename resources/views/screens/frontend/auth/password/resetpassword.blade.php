...
@section('content')
    ...
    <div>
        ...
        @endif
            <form action="{{route('reset-password.process-password', [request('userId'), request('token')])}}" method="POST" class="lg:p-10 p-6 space-y-3 relative bg-white shadow-xl rounded-md">
                @csrf
                @method('POST')
                <h1 class="lg:text-2xl text-xl font-semibold mb-6"> Reset Password </h1>
                <div>
                    <label class="mb-0" for="password"> Password </label>
                    <input type="password" id="password" name="password" value="{{old('password')}}" placeholder="******" class="bg-gray-100 h-12 mt-2 px-3 rounded-md w-full border-transparent">
                    @error('password')
                ...
                <div>
                    <label class="mb-0" for="password"> Password Comfirm</label>
                    <input type="password" id="password" name="password_confirm" value="{{old('password_confirm')}}" placeholder="******" class="bg-gray-100 h-12 mt-2 px-3 rounded-md w-full border-transparent">
